# Investigation Notes — Sentinel Lab 09

## Objective

The objective of this lab was to take a validated authentication detection query and operationalize it as a **Microsoft Sentinel Scheduled Analytics Rule**.

The detection focuses on repeated authentication failures against multiple accounts from the same source IP, followed by a successful authentication.

Because suitable persistent authentication telemetry was not available in the workspace, the investigation used synthetic events generated with `datatable()`.
...
The KQL detection successfully identified the intended synthetic authentication pattern and was converted into a scheduled Sentinel analytics rule.

The lab demonstrates the transition from manually testing detection logic to configuring an operational Sentinel detection workflow.

The result should be treated as a **simulated detection** because the underlying telemetry was generated with `datatable()` rather than collected from a real authentication source.
